...
try:
    ...
    from Jarvis.M2_VTTV.VTTV import text_voice as v
    from Jarvis.M3_Pwriter.fwrite import file_op as fop
    from Jarvis.M3_Pwriter.formatter import format as fw, dr_detail as dd
    from Jarvis.M7_editor.texteditor import*
    from Jarvis.M4_Database.dbOperations import Database_Insert as di, Database_Retrieve as dr, Database_update as up, Database_delete as de
    from Jarvis.M10_csvfile.tocsv import tocsvfile as t
...
def askformail():
        ...
        while True:
            ...
            if 'y' in str2 or 'jarvis' in str2 or 'send' in str2:
                ...
                msg['From']='gehurecruitpreetichand@gmail.com'
                doc = "Thanks and regards\n"

       
                str1 = ""
                with open("Jarvis/pers.txt") as f:
                    r = f.readlines()
                ...
                s.starttls()
                s.login('', '')
                s.send_message(msg,msg['From'],msg['To'])
...
def main(query):
    ...
    if "jarvis" in query:
        ...
        v.speak("Hello I am Jarvis. Please tell me how may I help you") 
        return

 
    if 'cription' in query:
    ...
    if 'my profile' in query:
        detail = dd.set_detail()
        fop.write_detail(detail)
        editor2()

    if 'retrieve' in query or 'document' in query:
        while True:
            dr.retrieve()
    ...
    if 'delete record' in query or 'remove record' in query:
        while True:
            de.delete()
    ...
    if "create" in query and "file" in query:
            speak(fileHandler.createFile(query), True, True)
            return
    ...
    if isContain(query, ['news']):
        ...
        query = record(False, False)
        if isContain(query["no", "don't"]):
    ...
    if isContain(query, ['wiki', 'who is']):
        Thread(target=webScrapping.downloadImage, args=(query, 1,)).start()
        speak('Searching...', True, True)
        result = webScrapping.wikiResult(query)
        showSingleImage('wiki')
        speak(result, True)
        return

    if isContain(query, ['time', 'date']):
        speak(normalChat.chat(query), True, True)
        return
...
if __name__ == '__main__':
    ...
    # Chat Frame
    chat_frame = Frame(root1, width=380, height=554, bg=chatBgColor)
    ...
    middle_right = Frame(root, bg=chatBgColor, width=850, height=700)
    middle_right.pack()
    middle_right.place(anchor='center', relx=0.66, rely=0.5)
    
    img = Image.open('extrafiles/images/home2.jpg')
    resized_image= img.resize((900,640), Image.ANTIALIAS) #639
    new_image= ImageTk.PhotoImage(resized_image)
    label = Label(middle_right, image = new_image)
    label.pack()
